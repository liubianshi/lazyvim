      ...
      fn = function(selects)
        ...
        if item:find("%.[Rr]$") then
          ...
          if item:find("^R%/") then
            ...
            path = vim.env.R_BOX_LIBRARY and vim.env.R_BOX_LIBRARY
              or vim.env.HOME .. "/Repositories/R-script/box/" .. item
          end
...
local keymap = function(key, fun, des, mode)
  mode = mode or "n"
  vim.keymap.set(mode, "<localleader>b" .. key, fun, { buffer = true, desc = des, noremap = true, silent = true })
end
