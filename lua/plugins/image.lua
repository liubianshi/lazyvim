return {
  { -- ferrine/md-img-paste.vim: paste image to markdown ---------------- {{{3
    ...
    opts = function()
      local in_blog = function(_)
        ...
        if parent_dir == "content/posts" or string.match(parent_dir, "^content/posts/") then
          return true
        else
          return false
        end
  ...
  { -- 3rd/image.nvim: Bringing images to Neovim. ----------------------- {{{2
    ...
    cond = not (vim.g.neovide and next(vim.g.neovdie)),
    ...
    config = function(_, opts)
      ...
      local image_is_rendered = function(id)
        ...
        for _, img in pairs(rendered_images) do
          if img.id == id and img.is_rendered and vim.tbl_contains(vim.api.nvim_list_wins(), img.window) then
            return img
          end
        end
      end
      ...
      local get_images_bind_to_win = function(win)
        ...
        local binded_images = vim.tbl_filter(function(img)
          if not img or not img.is_rendered then
            return false
          end
          ...
          return false
        end, require("image").get_images() or {})
      ...
      local centered_image_in_popup_window = function(img)
        if not img or not img.is_rendered or vim.api.nvim_win_get_config(img.window).relative == "" then
          return
        end
      ...
      local resize_popup_image = function(img, position)
        ...
        if not pop_win_config or pop_win_config.relative == "" then
          return
        end
        ...
        local win_config = vim.tbl_extend("force", pop_win_config, { width = adjust_win_width, col = adjust_win_col })
      ...
      local gen_id_with = function(file)
        ...
        if vim.fn.filereadable(file) == 0 or not vim.tbl_contains({ "png", "jpeg", "jpg", "gif", "webp" }, ext) then
          return nil
        end
      ...
      local generate_image = function(file, image_id, linenr)
        ...
        if display_row > 1 then
          display_opts.y = display_row
        end
      ...
      local display_image = function(file, opt)
        ...
        opt = vim.tbl_extend("keep", opt or {}, opts_default)
        ...
        if preview_image and not opt.update then
          return
        end

        open_window_for_preview(opt.method)
        vim.schedule(function()
          preview_image = generate_image(file, image_id, opt.line)
      ...
      vim.api.nvim_create_autocmd({ "WinResized" }, {
        ...
        callback = function(ev)
          if vim.api.nvim_win_get_config(ev.match + 0).relative ~= "" then
            return
          end
